...
import com.github.joaogalli.capstonepluris.posts.WebViewActivity;
...
public class PostsAppWidgetProvider extends AppWidgetProvider {

    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        // Perform this loop procedure for each App Widget that belongs to this provider
        for (int appWidgetId : appWidgetIds) {
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_posts_details);

            // Create an Intent to launch MyStocksActivity
            Intent intent = new Intent(context, SubredditListActivity.class);
            // TODO enviar qual é o subreddit
    ...
    public void onReceive(@NonNull Context context, @NonNull Intent intent) {
        ...
        if (PostCursorAdapter.ACTION_DATA_UPDATED.equals(intent.getAction())) {
    ...
    private void setRemoteAdapter(Context context, @NonNull final RemoteViews views) {
        views.setRemoteAdapter(R.id.widget_list,
                new Intent(context, PostsWidgetIntentService.class));
    }
